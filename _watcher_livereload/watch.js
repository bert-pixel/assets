...
if (ENABLE_LIVERELOAD === 'true') {
  try {
    ...
    console.log(`[livereload] listening on ws://localhost:${LIVERELOAD_PORT} (enable the LiveReload extension on the remote page's tab)`);
  } catch (err) {
    console.warn(`[livereload] failed to start (uploads will still work): ${err.message}`);
  }
}
